...
function rewardPerToken() public view returns (uint256){
  ...
  return s_rewardPerTokenStored + (((block.timestamp - s_lastUpdateTime) * REWARD_RATE * 1e18)/ s_totalSupply);
          ...
          // do we allow any tokens? if yes (uint256 amount, token address)
          //a specifc token? correct.
...
//
//the contract is going to emit X tokens per second and disperse them to all stakers
